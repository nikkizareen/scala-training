 case class Vehicle (vehicleid:Int, lat:Double, long: Double, Model:String){
  def state: String = (
   ...
   val vehicle: List[Vehicle] = List(vehicle1, vehicle2, vehicle3, vehicle4, vehicle5, vehicle6, vehicle7, vehicle8, vehicle9, vehicle10)

 case class VehicleState( vehicleid:Int, lat:Double, long: Double, Model:String, State:String)
 val result :List [VehicleState]= vehicle.map(x => new VehicleState(x.vehicleid, x.lat, x.long, x.Model, x.state))

  result.filter(x => x.State == "Texas")
